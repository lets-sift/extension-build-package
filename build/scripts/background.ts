...
chrome.runtime.onMessage.addListener(
    (message, sender, sendResponse) => {
        ...
        if (message.message === "recommend-products") {
            ...
            const frontendProduct: FrontendProduct = message.payload;
            // TODO: clean up
            ...
            const isLoggedIn = async () => {
                ...
                try {
                    const serverResponse = await fetch(baseUrl + "getUserInfo", requestOptions);
                    console.log(serverResponse)
                    if (serverResponse.status == 200) {
                        const response: any[] = await getUserInfo(baseUrl);
